...
$opt[1] = '--vertical-label % --title "' . $hostname . ' / ' . $servicedesc . '" --lower-limit=0 --upper-limit=100 --rigid';
$ds_name[1] = 'Disk busy';
$def[1] = "";
...
foreach ($this->DS as $KEY=>$VAL) {
    ...
    $VAL['NAME'] = str_pad($VAL['NAME'],46,' ',STR_PAD_RIGHT);
    $def[1] .= "LINE1:var" . $KEY . $colors[$KEY] . ":\"" . $VAL['NAME'] . "\" ";
    $def[1] .= "GPRINT:var" . $KEY . ":LAST:\"Last\: %6.2lf%s%%\" ";
    #$def[1] .= "GPRINT:var" . $KEY . ":MIN:\"  Min\: %6.2lf%s%%\" ";
    #$def[1] .= "GPRINT:var" . $KEY . ":AVERAGE:\"Avg\: %6.2lf%s%%\" ";
    $def[1] .= "GPRINT:var" . $KEY . ":MAX:\"Max\: %6.2lf%s%%\\n\" ";
}

$def[1] .= 'COMMENT:"Template for\: check_busy\r" ';
$def[1] .= 'COMMENT:"Check Command ' . $VAL['TEMPLATE'] . '\r" ';
?>
