...
foreach ($this->DS as $KEY=>$VAL) {
    if ( preg_match('/TEMP/',$VAL['NAME']) == 1 ) {
        $temp_sources[] = $KEY + 1;
        }
    elseif ( preg_match('/FAN/',$VAL['NAME']) == 1 ) {
        $fan_sources[] = $KEY + 1;
        }
    else {
...
foreach ($temp_sources as $i) {
	...
	$opt[$graph_count] .= ' --watermark="Template: check_brocade_env.pl by Yannick Charton"'; 
        $def[$graph_count] = "";
        // Then add ds info to the graph
        $def[$graph_count] .= "DEF:var" . $source_count . "=$RRDFILE[$i]:$DS[$i]:AVERAGE ";
        $def[$graph_count] .= "LINE1:var" . $source_count . $temp_colors[$source_count] . ":\"$NAME[$i] \" ";
    ...
    // Finished to integrate the ds related to the current graph
    //if($source_count == sizeof($temp_sources);) {
    //    $source_count = 0;
    //}
}
...
foreach ($fan_sources as $i) {
    ...
    if($source_count == 1) {
        ...
        $opt[$graph_count] = '--vertical-label "' . $vlabel . '" --title "Fans on ' . $hostname . '" --lower-limit=0 --rigid';
        $opt[$graph_count] .= ' --watermark="Template: check_brocade_env.pl by Yannick Charton"';
        $def[$graph_count] = "";
        // Then add ds info to the graph
        $def[$graph_count] .= "DEF:var" . $source_count . "=$RRDFILE[$i]:$DS[$i]:AVERAGE ";
        $def[$graph_count] .= "LINE1:var" . $source_count . $fan_colors[$source_count] . ":\"$NAME[$i] \" ";
        $def[$graph_count] .= "GPRINT:var" . $source_count . ":LAST:\"last\:%5.0lf $vlabel \" " ;
        $def[$graph_count] .= "GPRINT:var" . $source_count . ":AVERAGE:\"avg\:%5.0lf $vlabel \" " ;
        $def[$graph_count] .= "GPRINT:var" . $source_count . ":MAX:\"max\:%5.0lf $vlabel \\n\" " ;
    }
